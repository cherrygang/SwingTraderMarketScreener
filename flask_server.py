...
stonk_search = mainObj(_month_cuttoff=month_cut,
                       _day_cuttoff=day_cut, _std_cuttoff=std_cut)
stonks = []
...
def create_flask_app(_refresh=False):
    ...
    # pulls SECRET_KEY from env var, else sets as 'detitaded_wam'
    SECRET_KEY = os.getenv('SECRET_KEY', 'deditated_wam')
    ...
    def refresh_wrapper(month_cut, day_cut, std_cut):
        rThread = Thread(target=refresh_thread,
                         args=(month_cut, day_cut, std_cut))
    ...
    def refresh_wrapper_wrapper():  # we have to go deeper
        ...
        while True:
            refresher_loop = Timer(
                interval=UPDATE_FREQ*60, function=refresh_wrapper, args=(month_cut, day_cut, std_cut))
